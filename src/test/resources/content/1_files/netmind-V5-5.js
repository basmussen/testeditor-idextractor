...
function cConfigPixel()
{
	...
	this.m_VcData = 'spVcData2';
	this.m_VcData_ttl = 60;				// 60 days
	this.m_VcTimeout = 'spVcTimeout';
	this.m_VcTimeout_ttl = 0.02083;		// 30 min
	this.m_VcPools = [{n:'0-3', l:0},{n:'4-6', l:4},{n:'7-9', l:7},{n:'10-15', l:10},{n:'15-20', l:16},{n:'21-30', l:21},{n:'31-45', l:31},{n:'46-60', l:46},{n:'61-90', l:61},{n:'91-150', l:91},{n:'151-X', l:151}];
...
//==========================================================
//methode zum Auslesen der VisitCount Parameter
//==========================================================
function remotepixel_getVcParameter () {
	...
	if (isValidVisit) {
		...
		var data=spNmGetCookie(vcDataCookie, viewMonth);
		if (data == null) {
		...
		if (data != null) {
			...
			if (cv.length == 2) {
				...
				if (cc.length == 2) {
					cookieMonth=parseInt(cc[0]);
					cookieCount=parseInt(cc[1]);
				}
				cookiePool=cv[1];
			}
			
			if (!isNaN(cookieCount) && !isNaN(cookieMonth) && cookiePool) {
				if (cookieMonth == viewMonth)
						cookieCount++;
					else 
						cookieCount = 1;

				var pool = null;
				for (key in this.m_oConfig.m_VcPools) {
					if (cookieCount >= this.m_oConfig.m_VcPools[key].l)
						pool = this.m_oConfig.m_VcPools[key].n;
				}

				if (pool != null && pool != cookiePool)
					this.addParameter("sp.vc2",pool);
...
cRemotePixel.prototype.getVcParameter	= remotepixel_getVcParameter;
...
function spNmGetCookie (name, valueBegin)
{
	...
	while (i < clen) {
		...
		if (document.cookie.substring(i, j) == arg) {
			var cookieVal=SPONgetCookieVal (j);
			if(typeof(valueBegin) !== 'undefined' && valueBegin != null) {
				if (cookieVal.indexOf(valueBegin) == 0)
					return cookieVal;
			}
			else
				return cookieVal;
...
function spNm(params) {
	if (typeof spNmParams == "undefined")
		spNmParams=params;

	var strURL=spNmSrc(params);
	if (strURL != null) {
...
function spNmReloadMergeParams(params) {
	var mergedParams = $.extend({}, spNmParams, params);
	spNmAjax(mergedParams);
}

function spNmAjax(params) {
	var strURL=spNmSrc(params);
	var image = new Image();
	image.src=strURL+'&d='+(Math.random()*100000);
}


function spIvwReload() {
	var img=document.getElementById("spIvwPixel");
	if (img != null && img.src != null) {
		var imgsrc=img.src;
		imgsrc=imgsrc.substr(0, imgsrc.indexOf("&d="));
		img.src=imgsrc+'&d='+(Math.random()*100000)
	}
