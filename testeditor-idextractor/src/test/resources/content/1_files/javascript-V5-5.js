...
SpOnENV_ForumSponDeServer = 'http:/'+'/app.forum.spiegel.de';
SpOnENV_SponUploadServer = 'http:/'+'/upl.spiegel.de';
SpOnENV_MapShowCategories = ('true' == 'true');

var _sf_startpt=(new Date()).getTime();/*! jQuery v1.11.1 | (c) 2005, 2014 jQuery Foundation, Inc. | jquery.org/license */
...
//				$('#home').length > 0 ?	$('.ui-tabs-panel').css('min-height','140px') : 
					...
					$.getJSON(url, function(json) {
					    $.each(json, function() {
					    	...
					    	if ( counter == '6' && url.match(/[0-9][.json]/) ) {
					    		// außer auf HP nur max 5 Elemente anzeigen
					    		$(".tabs .load-icon").hide();
					    		return false;
					    	}
	...
	// facebook Info-Overlay
	
	$('.facebook-info-button a').hover(
		function(){
			$(this).parent().next('.js-facebook-info-text').show();
		},
		function(){
			$(this).parent().next('.js-facebook-info-text').hide();
	});

	// QC-Belegungen in Ressort-Boxen auf HP
	hpRessortAd();
...
/**
 * QC-Belegungen in Ressort-Boxen
 */
hpRessortAd = function() {
	$('.ressort-teaser-box-body .column-small').each(function(i) {
		if($(this).find('div[class^="js-ressort-ad"]').attr('class') && $(this).find('.spCommercial').length > 0) {
			$(this).find('.js-ressort-content').remove();
			$(this).find('div[class^="js-ressort-ad"]').show();
		} else if ($(this).find('div[class^="js-ressort-ad"]').attr('class') && spHashParams["oas.showpos"] == '1') {
			$(this).find('div[class^="js-ressort-ad"]').show();
		}
	});
};
...
$.spHpSlideBox = function(obj, callback, param){
	...
	var src = "/fragments/centerpage/" + ressort + ".json";
...
$.spHpSlideBoxCallback = function(obj, param) {
	var ressort = $(obj).find('.ressort-slide').attr('rel');
	spNmReloadMergeParams({df : 'ressortbox_'+ressort});
	spIvwReload();
}
...
function topPosterInit(hashParams) { 
	...
	if (typeof hashParams != 'undefined' && hashParams["ai"] != null) {
		...
		if (device != null && device.target == 'mobileplus' && !spUA.isiPad()) {
			document.location.href="/artikel/a-"+hashParams["ai"]+".html#ref="+encodeURIComponent(hashParams["ref"]);
			return;
		}
    ...
    $('.nav-channel li[class$="-border-bottom"]').hover(function(){
    	...
    	if($currentItem.find('.nav-channel-sub-assets .js-nav-channel-sub-teaser').length < 1) {
	    	$currentItem.find('.asset').each(function(){
	    		...
	    		$(this).parent().find('.'+aid).load("/fragments/centerpage/"+ aid +".html");
	    	});
    	}
...
var spAdColMoved = false;

function spMoveAdCol(){
	if($("#spColumnAd").length > 0){
		var superLayer	= $("#spColumnAd").html().indexOf("qcSuperLayer");
		var wrapperPos	= Math.round($("#wrapper").offset().top);
...
function spDelegate(forceDelegation) {
	if (spMobilePlusUrl && spOldMobileUrl) {
		...
		var spMobileHpCompact = SPONgetCookie('spMobileHpCompactView');
		if (spMobileHpCompact != null && spMobileHpCompact == 'true') {
			if (spMobilePlusUrl == '/index.html')
				spMobilePlusUrl='/index-compact.html';
		}
...
function spStartVideo4(videoId, variant, poster, prerollGateway, postrollGateway, allowAds, credit, displaycat, videocat, server, trackingsize, startmod, onAdStart, onAdEnd, onVideoEnd, onFinalEnd, embedWidth, embedHeight){
	...
	try {
		var hashKeineWerbung=hashParams["adition.keinevideowerbung"];
		if (hashKeineWerbung == "1") {
			prerollGateway = "";
			postrollGateway = "";
		}
	} catch (e) {}
	...
	// 
	try {
		var oasVideoBelegung=hashParams["oas.videobelegung"];
		if (oasVideoBelegung != null && oasVideoBelegung == 'keinewerbung') {
			prerollGateway = "";
			postrollGateway = "";
		}
	} catch (e) {}
	...
	try {
		var hashVideoBelegung=hashParams["adition.videobelegung"];
		if (hashVideoBelegung != null) {
			if (prerollGateway) 	prerollGateway  += encodeURIComponent("&prf[videobelegung]=" + hashVideoBelegung);
			if (postrollGateway) 	postrollGateway += encodeURIComponent("&prf[videobelegung]=" + hashVideoBelegung);
		}
	} catch (e) {}
	
	
	// ref-Parameter fuer Statistik:
	try {
		var refParamRead=hashParams["ref"];
		if (refParamRead != null) {
			refParam = refParamRead;
		}
	}
	catch (e) {}	
	...
	if (spUA.isIDevice() || spUA.isAndroid()){
		...
		if (parseInt(videoId) >= 58671) {
			$('#spReplaceVideo').sponVideoPlayer({
				...
				JSONP_Callback_PreRoll : 'jason_callback',
				...
				JSONP_Callback_PostRoll :  'jason_callback',
				...
				netmind : {
					start : 'http://count.spiegel.de/nm_trck.gif?url=' + url + '&referrer=' + referrer + '&ref=' + refParam + '&sp.atyp=33&sp.site=1&sp.videocredit=' + credit + '&sp.df=' + trackingsize + '&sp.videodisplaycatid=' + displaycat + '&sp.szw=1218&sp.videocatid=' + videocat + '&sp.aid=' + videoId + '&sp.channel=781',
					// start : '/static/sys/ipad-video-special/track.gif?t=netmind_start',
					
					// ... url & referrer sind undefined!
					end  : 'http://count.spiegel.de/nm_trck.gif?url=' + url + '&referrer=' + referrer + '&sp.atyp=33&sp.site=1001&sp.event=end&sp.videocredit=' + credit + '&sp.df=' + trackingsize + '&sp.videodisplaycatid=' + displaycat + '&sp.szw=1218&sp.videocatid=' + videocat + '&sp.aid=' + videoId + '&sp.channel=781'
					// end: '/static/sys/ipad-video-special/track.gif?t=netmind_end'
				},
				...
				onEnd: (variant === 'inline' ? 'onFinalEnd' : null)
				

			}); // calls the init method

			// INIT FINAL SCREEN
			$('.spWatchVideoAgain').on('click', function () {
		...
		var pversion = sp_webcfg_global.flash.pluginversion.major + "." + sp_webcfg_global.flash.pluginversion.minor;
		swfobject.embedSWF("/static/flash/flashvideo/" + sp_webcfg_global.flash.player, "spReplaceVideo", embedWidth, embedHeight, pversion, "", PlayerFlashVars, PlayerParams);
	} else {
		$('#sponiPadPlayer').hide();
		var spNoFlashPlayer = document.getElementById('spNoFlashPlayer');
		var fpversion = swfobject.getFlashPlayerVersion();
		var spNoFlashPlayerVersionHint = document.getElementById('spNoFlashPlayerVersionHint');
		spNoFlashPlayerVersionHint.innerHTML = 'Dieses Video benötigt mindestens Version ' + sp_webcfg_global.flash.pluginversion.major + "." + sp_webcfg_global.flash.pluginversion.minor + ' des Flash-Plugins. Gefundene Version: ' + fpversion.major + "." + fpversion.minor + "." + fpversion.release + ".";
		spNoFlashPlayer.setAttribute("style", "display:block;");
	}
}

function spRestartVideo(){
	// Vars wurden von spStartVideo4 gesetzt
	// fuer HTML5-Player reload
	if (typeof PlayerFlashVarsRestart != 'undefined') {
		var pversion = sp_webcfg_global.flash.pluginversion.major + "." + sp_webcfg_global.flash.pluginversion.minor;
		swfobject.embedSWF("/static/flash/flashvideo/"  + sp_webcfg_global.flash.player, "spReplaceVideo", embedWidth, embedHeight, pversion, "", PlayerFlashVarsRestart, PlayerParams);
	} else {
...
function onContentVideoError(){
	var endScreen = document.getElementById('spVideoEndScreen').style.display;
    $('.watch-video-again').css({display: 'none'});
    $('#spVideoEndScreen').append('<p style="font-weight:bold;color:white" class="watch-video-again">Dieses Video ist leider an Ihrem Standort nicht verfügbar.</p>');
...
function spShowVideoShortUrl(){
	var checkLayers = document.getElementById('js-popup-layer').firstChild;
	if(checkLayers != null){
...
function spOasSetReminder(iframeUrl){
	...
	if (iframeUrl) {
		...
		var iframe = this.parent.document.getElementById("video-iframe");
		if (!iframe == false)
...
function spConfigureVideoEmbed(videoId, eingabe){
	try {
		...
		if (errorMsg == ''){
			...
			userSource = $('#spVideoEmbedSourceArea').val('<iframe width="' + userWidthValue + '" height="' + userHeightValue + '" frameborder="0" scrolling="no" border="0" src="http:/' + '/www.spiegel.de/video/embedurl/video-' + videoId + '-' + userWidthValue + '_' + userColorValue.substring(1) + '_' + userBgColorValue.substring(1) + '.html"></iframe>');
		} else {
		...
		if (eingabe == 'reset'){
			...
			userSource = $('#spVideoEmbedSourceArea').val('<iframe width="640" height="531" frameborder="0" scrolling="no" border="0" src="http:/' + '/www.spiegel.de/video/embedurl/video-' + videoId + '-640_000000_ffffff.html"></iframe>');
		}
	}
	catch (e) {}
}
...
function spOpenEmbedForm(assetId){
	...
	spOpenPopupLayer2('js-embed-video-popup-layer', null, headline, framelocation);
...
function spOpenPopupLayer2(classname, openerObject, headline, framelocation)  {
	...
	var html = '<a href="javascript:spClosePopupLayer();" class="close-button"><img src="'+SpOnENV_SERVER+'/static/sys/v10/buttons/btn_close_black.png" class="close-button" width="25" height="25" alt="Schlie&szlig;en" /><\/a><h1>' + headline + '</h1>';
...
function spMSLogin2(openerObject, backUrl, feature, language, silent)  {
	var headline;
	var framelocation = SpOnENV_SERVER_SSL;
	var pagename;
	if (language == 'en') {
		framelocation += '/international/login/';
		headline = '<img src="'+SpOnENV_SERVER+'/static/sys/v10/meinspiegel/logo_my_spiegel.png" id="ms-logo" alt="My SPIEGEL" />';
	} else {
		framelocation += '/meinspiegel/';
		headline = '<img src="'+SpOnENV_SERVER+'/static/sys/v10/meinspiegel/logo_mein_spiegel.png" id="ms-logo" alt="Mein SPIEGEL" />';
		language = 'de';
	}
	if (silent) {
...
function spUpdateMSLoginLinks()  {
	...
	if ( language=='en' )	{
		if ( spMSisLoggedIn() )	{
			ll += '<li><a href="' + SpOnENV_SERVER + '/international/login/logout.html?backUrl=' + escape(window.location.href) + '">Sign out<\/a>|<\/li>';
			ll += '<li><a href="' + SpOnENV_SERVER + '/international/login/index.html">My SPIEGEL</a><\/li>';
		} else {
			ll += '<li><a onclick="spMSLogin2(null, null, null, \'en\'); return false;" href="' + SpOnENV_SERVER + '/international/login/login.html">Sign in<\/a>|<\/li>';
			ll += '<li><a href="' + SpOnENV_SERVER + '/international/login/register.html">Register<\/a><\/li>';
		}
	} else {
		
		if ( spMSisLoggedIn() )	{
			ll += '<li><a href="' + SpOnENV_SERVER + '/meinspiegel/logout.html?backUrl=' + escape(window.location.href) + '">Logout<\/a>|<\/li>';
			ll += '<li><a href="' + SpOnENV_SERVER + '/meinspiegel/index.html">Mein SPIEGEL</a>|<\/li>';
			if ( SPONgetCookie('wiWlNumBookmarks') && /^[1-9][0-9]*$/.test(SPONgetCookie('wiWlNumBookmarks')) ) {
				ll += '<li><a href="' + SpOnENV_SERVER + '/meinspiegel/merkliste/index.html">Merkliste (' + SPONgetCookie('wiWlNumBookmarks') + ')<\/a><\/li>';
			} else {
				ll += '<li><a href="' + SpOnENV_SERVER + '/meinspiegel/merkliste/index.html">Merkliste<\/a><\/li>';
			}
		} else {
			ll += '<li><a onclick="spMSLogin2(); return false;" href="' + SpOnENV_SERVER + '/meinspiegel/login.html">Login<\/a>|<\/li>';
			ll += '<li><a href="' + SpOnENV_SERVER + '/meinspiegel/artikel/a-703606.html">Registrierung<\/a><\/li>';
		}
	}
	$links.html(ll);
}
...
$(document).ready(function() {
 ...
 function spOpenSendForm(language, which, element)  {
	var headline;
	var framelocation;
	if (which == 'video') {
...
function spOpenFeedbackForm(scrollelem, language)  {
	var headline;
	var framelocation;
	framelocation = SpOnENV_SERVER + '/feedback/index-' + language + '.html';
	if (language == 'en') {
...
function spBlogOpenForm(fromElement, forumresponse) {
	...
	$("#js-comments-box-form-entry-submit").bind('click', 	function(event){spBlogFormSubmit($("#js-article-comments-box-form form"));});
...
function spBlogShowMessage(cssClass, message) {
	if (typeof console != "undefined" && typeof console.log != "undefined") console.log('spBlogShowMessage: ' + message);
	if (message == '') {
		$("#js-message").html('');
...
// ----- social bookmarks Auklapper: ----- 
function spInitSocialOther() {
	if( ! $('#js-social-other-poster').length ) {
		return;
	}
	$('#js-social-other-poster').find('ul').hide();
	$('#js-social-other-poster').hover(function(){
		$(this).find('ul').show();
	},
	function() {
...
function spDoVeeseo(veeseoArticleid, veeseoDate, veeseoChannelid, veeseoChannelname) {
	...
	if(typeof veeseoDiv == 'object' && veeseoArticleid != '' && veeseoChannelid != '') {
		...
		if(sp_webcfg_global != undefined) {
			...
			if (vp_type != '') {
				if (vp_type == 'veeseo') {
					...
					vp_url = 'http:/'+'/farm.plista.com/iframeWidget.php?publickey=spiegel&widgetname=belowArticle&c='+encodeURI(veeseoChannelname)+'&item[objectid]='+veeseoArticleid+'&item[url]=http:/'+'/www.spiegel.de/artikel/a-'+veeseoArticleid+'.html&item[updated_at]='+veeseoDate;	
					vp_height= '1200px';
				}
		...
		var veeseo_config;
		var veeseo_url = '';
		if(sp_webcfg_global != undefined && sp_webcfg_global.veeseo == 'test' && typeof sp_webcfg_global.veeseotest != 'undefined') {
			veeseo_config = sp_webcfg_global.veeseotest[veeseoChannelid]
			if(veeseo_config != undefined) {
				...
				z -= veeseo_config[0];
				if(z <= 0) {
					veeseo_url = '/video/veeseo/most-'+veeseoChannelid+'.html';
				} else if (z <= veeseo_config[1]) {
					veeseo_url = '/video/veeseo/last-'+veeseoChannelid+'.html';
				} 
			}
		} else {
...
function spnetDynamicFrame(read_cfg, get_element, new_id, new_width, new_height, new_url) {
	var config_activated = false;
	if (read_cfg == 'always') {
		config_activated = true;
	}
	if (sp_webcfg_global.social_bm) {
		if( read_cfg == 'social_fbshare' && sp_webcfg_global.social_bm.fbshare == true) {
			config_activated = true;
		}
		else if( read_cfg == 'social_fb' && sp_webcfg_global.social_bm.fb == true) {
			config_activated = true;
		}
		else if( read_cfg == 'social_tw' && sp_webcfg_global.social_bm.tw == true) {
			config_activated = true;
		}
		else if( read_cfg == 'social_gplus' && sp_webcfg_global.social_bm.gplus == true) {
			config_activated = true;
		}
	}
	if (config_activated) {
		var my_element;
		if( my_element = document.getElementById(get_element) ) {
			var my_frame = document.createElement("iframe");
			my_frame.id                = new_id;
			my_frame.style.width       = new_width;
			my_frame.style.height      = new_height;
			my_frame.src               = new_url;
			my_frame.marginHeight      = "0";
			my_frame.marginWidth       = "0";
			my_frame.scrolling         ="no" ;
			my_frame.style.border      = "none";
			my_frame.frameBorder       ="0"; 
			my_frame.style.overflow    = "hidden";  
			my_frame.allowTransparency ="true";
			my_element.appendChild(my_frame);
		}
	}
}
...
function spDoGa() {
	...
	if( sp_webcfg_global.google_analytics != undefined && sp_webcfg_global.google_analytics == true) {
		_gaq.push(['_setAccount', 'UA-26045149-1']);
		_gaq.push(['_setDomainName', 'spiegel.de']);
...
//				console.log("NOTIFIER NOT STARTED (no permission)");
			...
			$('.notifierstartbutton')	
				.text('Ihr Browser unterstützt diese Funktion nicht')
				.addClass('disabled')
				.unbind('click');
...
//		console.log("REFRESH");
		that.getTops(function(tops) {
			...
			for (var i=0; i<notify.length; i++) {
				...
				$.ajax({
					type: "GET",
					url: SpOnENV_SERVER + "/fragments/article/a-"+aid+".json",
					success: function(data) {
...
//		console.log("GET TOPS...");
		$.ajax({
			type: "GET",
			url: SpOnENV_SERVER + "/jsapi/notification/top.json",
			success: function(data) {
...
//		console.log("Requesting permission for notifications.");
		if (that.notificationType == 1) {
			...
			var c = confirm("Möchten Sie einen Hinweis bei neuen Nachrichten auf unserer Startseite bekommen?");
...
function spStartVideo3(videoId, poster, prerollGateway, postrollGateway, belegung, allowAds, credit, displaycat, videocat, server, trackingsize, startmod, onAdStart, onAdEnd, onVideoEnd, onFinalEnd, embedWidth, embedHeight){
	// mit Delivery 5.5:  spStartVideo3 => spStartVideo4: belegung weg, Variant hinzu
	spStartVideo4(videoId, "oldVideo3", poster, prerollGateway, postrollGateway, allowAds, credit, displaycat, videocat, server, trackingsize, startmod, onAdStart, onAdEnd, onVideoEnd, onFinalEnd, embedWidth, embedHeight);
}
